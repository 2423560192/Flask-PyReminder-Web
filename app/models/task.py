...
class Task(Base):
        ...
        if r:
            try:
                ...
                if all_tasks:
                    ...
                    # 按日期时间排序
                    return sorted(tasks_list, key=lambda x: x.get("datetime", datetime.datetime.max.replace(
                ...
                if hasattr(config, 'TASKS_KEY'):
                    ...
                    if tasks_json:
                        try:
                            ...
                            # 按日期时间排序
                            return sorted(stored_tasks, key=lambda x: x.get("datetime", datetime.datetime.max.replace(
                                tzinfo=datetime.timezone.utc)))
                        except Exception as e:
    ...
    def update_task_status(cls, task_id, triggered=True):
        ...
        if r:
            ...
            if triggered:
                ...
                # 如果未触发，重新添加到待处理队列
                task_datetime = datetime.datetime.fromisoformat(task['datetime']) if 'datetime' in task and task[
                    'datetime'] else None
                if task_datetime:
    ...
    def sync_to_postgres(cls):
        ...
        try:
            ...
            synced_tasks = {task_id.decode('utf-8') if isinstance(task_id, bytes) else task_id for task_id in
                            synced_tasks}
    ...
    @classmethod
    def get_all_tasks(cls, username=None, include_triggered=True, force_refresh=False):
            ...
            try:
                ...
                if r.exists(TASKS_KEY):
                    try:
                        ...
                        # 如果成功从Redis获取数据并需要强制刷新，直接返回结果
                        if stored_tasks or force_refresh:
                            print(f"从Redis获取到 {len(stored_tasks)} 个任务数据")
                            # 按时间排序
                            return sorted(stored_tasks, key=lambda x: x.get("datetime", datetime.datetime.max.replace(
                                tzinfo=datetime.timezone.utc)))
                ...
                # 如果哈希表不存在或者获取失败，但是需要强制刷新，则跳过后续查询
                if force_refresh:
                    print("强制刷新模式：Redis中没有找到任务数据")
                    return []
        ...
        # 2. 如果Redis获取失败且不是强制刷新模式，尝试从数据库获取
        ...
        # 3. 最后，如果所有远程存储都失败或者是强制刷新模式但Redis失败，从内存获取
        # 注意：在强制刷新模式下，如果Redis获取失败，这里会返回内存中的数据，可能不是最新的
        if username:
            filtered_tasks = [task for task in tasks if task.get("owner") == username]
            if not include_triggered:
                filtered_tasks = [task for task in filtered_tasks if not task.get("triggered", False)]
            return filtered_tasks
        
        if not include_triggered:
            return [task for task in tasks if not task.get("triggered", False)]
    ...
    def get_pending_tasks(cls):
        ...
        return sorted(pending_tasks,
                      key=lambda x: x.get("datetime", datetime.datetime.max.replace(tzinfo=datetime.timezone.utc)))
